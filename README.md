...
- props로 전달하는 state를 줄여보자
- 어떤 컴포넌트든 자유롭게 접근하는 전역 state를 생성
- todos를 관리하는 것은 일반적으로 적당하지 않음
- 회원정보, 장바구니, 테마 등이 적당한 것으로 판단

## 2. 단계별 실습

### 2.1. Context와 useReduce 조합
...
// - action 의 모양 {type:string, payload: {id:"", title:"", completed:false}}
...
### 2.2. useCallback으로 함수를 리랜더링에서 제외하기
...
### 2.3. useMemo로 state를 리랜더링에서 필요한 경우만 업데이트하기
...
- /src/context/todo/`TodoProvider.tsx`
